...
if ~isempty(plannedPath)
    ...
    for idx = 1:skip:nPoints
        ...
        patch( Xc, Yc, Zc,          ... % Corner Coordinate
              [0 0.5,1],            ... % Blue side Color (RGB)
              'FaceAlpha', 0.2,     ... % a bit transparency (0 ~ 1)
              'EdgeColor', 'none',  ...
              'Parent', mapAxes);       % Draw at mapAxes

        % If u think rectangle is too small ;
        % Draw as torsoHeight +- 0.01 ; a bit thickness
    end
